...
if ( (count($_POST) != 0) && (isset($_REQUEST["Submit"])) && ($_REQUEST["Submit"]=="Shift Now") )
{
	...
	if($errcnt == 0)
	{
		...
		{
			mysql_query("update sbjbs_resume_locs set sb_loc_id=".$_REQUEST["cat1"]." where sb_loc_id=".$_REQUEST["loc_id"]);
			$sbresume_count=mysql_affected_rows();
		}

		if(isset($_REQUEST["sbjobs"]) && ($_REQUEST["sbjobs"] == 'Yes') )
		{
			mysql_query("update sbjbs_job_locs set sb_lid=".$_REQUEST["cat1"]." where sb_lid=".$_REQUEST["loc_id"]);
...
function main()
{
	...
	$list_num=mysql_num_rows(mysql_query("select * from sbjbs_resume_locs where sb_loc_id=$loc_id"));
	$list_num1=mysql_num_rows(mysql_query("select * from sbjbs_job_locs where sb_lid=$loc_id"));
...
<table width="90%" border="0" align="center" cellpadding="2" cellspacing="10">
  <tr> 
    <td height="25" bgcolor="#004080"><font color="#FFFFFF" size="2" face="Arial, Helvetica, sans-serif"><strong>&nbsp;Shift 
      ...
      <?php 
	  $sbq_cat="select * from sbjbs_locations where sb_id=".$_REQUEST["loc_id"]; 
	  $sbrow_cat=mysql_fetch_array(mysql_query($sbq_cat));
	  echo $sbrow_cat["sb_loc_name"];
	  ?>
      </font> to <font color="#FFcc00"> 
      <?php $sbq_cat="select * from sbjbs_locations where sb_id=".$_REQUEST["cat1"]; 
	  $sbrow_cat=mysql_fetch_array(mysql_query($sbq_cat));
	  echo $sbrow_cat["sb_loc_name"];
	  ?>
      </font></strong></font></td>
  ...
  <form name="form2" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" onSubmit="return validate_form(this);">
    ...
    <tr> 
	  ...
	  <input name="Submit" type="submit" id="Submit" value="Shift Now"> 
        ...
        <input name="loc_id" type="hidden" id="loc_id" value="<?php echo $_REQUEST["loc_id"]; ?>"> 
      </td>
    </tr>
  </form>
</table>
...
<table width="90%" border="0" align="center" cellpadding="2" cellspacing="10">
  <tr> 
    <td height="25" bgcolor="#004080"><font color="#FFFFFF" size="2" face="Arial, Helvetica, sans-serif"><strong>&nbsp;Following 
      ...
      <?php $sbq_cat="select * from sbjbs_locations where sb_id=".$_REQUEST["loc_id"]; 
	  $sbrow_cat=mysql_fetch_array(mysql_query($sbq_cat));
	  echo $sbrow_cat["sb_loc_name"];
	  ?>
      </font> to <font color="#FFcc00"> 
  ...
  <tr> 
    <td valign="top" bgcolor="#F5F5F5"> <font size="2" face="Arial, Helvetica, sans-serif">Click 
      <a href="browselocs.php?loc_id=<?php echo $loc_id; ?>">here</a> 
      to go back to Manage Locations Section</font></td>
  </tr>
</table>
<?php
	}
