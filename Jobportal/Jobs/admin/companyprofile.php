...
{
	header("Location: adminhome.php?msg=".urlencode("Invalid Id, cannot contimue"));
	die();
}
...
//		$sbuserid=$_SESSION["sbjbs_emp_userid"];
		...
		$insert_query="insert into sbjbs_companies (sb_name,sb_logo,sb_type,sb_cid, sb_location, sb_country,  sb_industry, sb_sales, sb_currency, sb_multiplier, sb_no_of_emps, sb_no_of_officies, sb_profile, sb_website, sb_viewed, sb_posted_on) values ('$companyname','$logo', $businesstype,'$sbcid_list', '$state', $sb_country, '$sb_industry' , '$sb_sales', $sb_currency, '$sb_multiplier', '$sb_no_of_emps', '$sb_no_of_officies', '$sb_profile', '$website', 0, $sbcurrent)";
//	die($insert_query);
		...
		{	//success
			...
			{	
				...
				header ("Location: adminhome.php?sb_type=3&id=$sb_offer_id&msg=".urlencode($sb_msg));
			}
			else
			{
...
///////////-------------------end mail to admin			
				header("Location: view_profile.php?sb_id=$sb_id&msg=".urlencode($sb_msg));
		...
		{	//failed
			header("Location: view_profile.php?sb_id=$sb_id&msg=".urlencode("Unable to update, please try again"));
			die();
		}	
...
*/			{	////admin 
				header ("Location: view_profile.php?id=$sb_id&msg=".urlencode($sb_msg));
			}
			die();
		}
		header("Location: view_profile.php?id=$sb_id&msg=".urlencode("Sorry, no updations carried out."));
		die();
		}
	}// end if no errs
}//if form posted
function main()
{
	global $sb_id, $errs, $errcnt;
	$sql="Select * from sbjbs_config";
	$rs0_query=mysql_query($sql);
	$rs0=mysql_fetch_array($rs0_query);

	$sb_cat_listing=$rs0["sb_cat_listing"];
...
$rs=mysql_query("Select * from sbjbs_companies where sb_id=$sb_id");
...
//{
//	echo "select * from sbjbs_categories where sb_id in ($cat_list)";
/*	$sbrs=mysql_query("select * from sbjbs_categories where sb_id in ($sbtemp_cid)");
	while($cat=mysql_fetch_array($sbrs))
	{
          ...
          <tr valign="top"> 
            ...
            <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr> 
                  ...
                  <td> <select name="state" id="state"  onChange="choose_country()" >
