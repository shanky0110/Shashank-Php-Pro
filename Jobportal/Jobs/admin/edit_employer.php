...
*/
		...
		elseif(mysql_num_rows(mysql_query("select * from sbjbs_employers where sb_email_addr='$email' and sb_id<>".$_REQUEST["id"]))!= 0)
	{
			$errs[$errcnt]="Some employer with same Email Address already exists";
    		$errcnt++;
	}
...
$rs_update=mysql_query($query_update);
	...
	{
		header("Location: employers.php?msg=".urlencode("Employer profile has been updated."));
		die();
	}
	else
	{
		header("Location: employers.php?msg=".urlencode("No updations carried out. Please try again!"));
		die();
	}
 }			//end if-errcnt==0
}			//end if count-post
...
$mem=mysql_fetch_array(mysql_query("select * from sbjbs_employers where sb_id=".$_REQUEST["id"]));
...
{
echo "<p>&nbsp;</p><p>&nbsp;</p><br><br><br><div align='center'><font class='normal'>Employer Not Found. Click <a href='index.php' >here</a> to continue</font></div><p>&nbsp;</p><p>&nbsp;</p><p>&nbsp;</p><p>&nbsp;</p><p>&nbsp;</p>";
return;
}
if  (count($_POST)>0)
{
...
<form name="form1" method="post" action="edit_employer.php" onSubmit="return validate(this);">
  <table width="90%" border="0" align="center" cellpadding="2" cellspacing="5" class="onepxtable">
    <tr class="titlestyle"> 
      <td colspan="3">&nbsp;Edit Personal Profile (Employer)</td>
    </tr>
    <tr valign="top"> 
