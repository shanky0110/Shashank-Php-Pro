...
$showform="";
	...
	{
		header("Location: jobs.php?msg=".urlencode("Invalid Id, cannot proceed!"));
		die();
	}
...
//	die($sbq_com); 
	...
	if(!$sbrow_com)
	{
		header("Location: gen_confirm.php?msg=".urlencode("You cannot post a job before creating company profile!"));
		die();
	}
*/
///////////--------------------------------------------------
...
{
	header("Location: gen_confirm.php?msg=".urlencode("You cannot post job due to lack of funds, please add some money first!"));
	die();
}
*/
...
{
		$errs[$errcnt]="You donot have sufficient funds, please either remove some paid options or add some money first";
   		$errcnt++;
//	header("Location: gen_confirm.php?msg=".urlencode("You cannot post job due to lack of funds, please add some money first!"));
//	die();
}
...
//			$cid_list=str_replace(";",",",str_replace("$","\$",addslashes($_REQUEST["cid1"])));		//converted to comman separated instead of semi colon separated
//			$loc_id_list=str_replace(";",",",str_replace("$","\$",addslashes($_REQUEST["loc_id"])));
//			$skl_id_list=str_replace(";",",",str_replace("$","\$",addslashes($_REQUEST["skl_id"])));
...
//			$cid_list=str_replace(";",",",str_replace("$","\$",$_REQUEST["cid1"]));
//			$loc_id_list=str_replace(";",",",str_replace("$","\$",$_REQUEST["loc_id"]));
//			$skl_id_list=str_replace(";",",",str_replace("$","\$",$_REQUEST["skl_id"]));
...
/*	
		...
		if(!$sbrow_com)
		{
			header("Location: gen_confirm.php?msg=".urlencode("Error, please create company profile first!"));
			die();
		}
*/		
...
//		if($sbrow_con["sb_job_approval"]=='auto')
		{
//			$sb_approved='yes';
		$query_update="update `sbjbs_jobs` set  sb_title='$sb_title', sb_role='$sb_role', sb_vacancies=$sb_vacancies, sb_experience=$sb_experience, sb_education='$sb_education', sb_min_salary=$sb_min_salary, sb_max_salary=$sb_max_salary, sb_currency=$sb_currency, sb_salary_type=$sb_salary_type, sb_career_level='$sb_career_level', sb_job_type=$job_type, sb_job_status=$job_status, sb_description='$sb_description', sb_cat_count=sb_cat_count+$sb_tobe_added_cat_count,  sb_loc_count=sb_loc_count+$sb_tobe_added_loc_count where sb_id=$sb_id";
			$msg="Job has been updated";
		}
/*		else
		{
			$sb_approved='no';
			$query_update="update `sbjbs_jobs` set  sb_title='$sb_title', sb_role='$sb_role', sb_vacancies=$sb_vacancies, sb_experience=$sb_experience, sb_education='$sb_education', sb_min_salary=$sb_min_salary, sb_max_salary=$sb_max_salary, sb_currency=$sb_currency, sb_salary_type=$sb_salary_type, sb_career_level='$sb_career_level', sb_job_type=$job_type, sb_job_status=$job_status, sb_description='$sb_description', sb_approved='$sb_approved', sb_cat_count=sb_cat_count+$sb_tobe_added_cat_count,  sb_loc_count=sb_loc_count+$sb_tobe_added_loc_count  where sb_id=$sb_id";
...
*/
			header("Location: jobs.php?msg=".urlencode($msg));
			die();
		}	//end if mysql_affected_rows() > 0 i.e.success
/*		else
		{
			header("Location: gen_confirm.php?msg=".urlencode("Some error occurred, please try again!"));
			die();
		}
*/	}			//end if-errcnt==0
}			//end if count-post
...
{		
	...
	if(!$sbrow_job)
	{
		header("Location: jobs.php?msg=".urlencode("No such job exists!"));
		die();
	}
...
function validate(form) 
{
	...
	if ( (form.sb_min_salary.value != "") && isNaN(form.sb_min_salary.value) ) 
	{
       	alert('Please specify numeric value for Minimum Salary!');
		form.sb_min_salary.focus();
		form.sb_min_salary.select();
	   	return false;
	}
	if ( (form.sb_max_salary.value != "") && isNaN(form.sb_max_salary.value) ) 
	{
       	alert('Please specify numeric value for Maximum Salary!');
