...
if(count($_POST)>0)
{
		...
		else
		{
			...
			sb_businesstype='$sb_businesstype' and sb_id<>".$_POST["id"]));
			if($rst)
			{
				$errmsg[$errcnt]="Business Type already exists in the list.";
				$errcnt++;
			}
		}
...
if($errcnt==0)
{
   ...
   mysql_query("update sbjbs_businesstypes set sb_businesstype='$sb_businesstype' 
	where sb_id=".$_POST["id"]);
	if(mysql_affected_rows()>0)
	{
	header("Location: "."manage_business_types.php?msg=".urlencode("Business Type has been updated."));
	die();
	}
	else
	{
	header("Location: "."edit_business_type.php?id=".$_POST["id"]."&msg=".urlencode("Some error occurred. Please try again."));
	die();
	}
}
}
...
$sb_businesstype=$cur["sb_businesstype"];
		  ...
		  onSubmit="return Validator();" >
            ...
            <tr class="row1"> 
              <td width="40%" align="right" bgcolor="#F5F5F5"> <div align="right"><font size="2" face="Arial, Helvetica, sans-serif"><strong> 
                  ...
                  Business Type:</strong></font></div></td>
