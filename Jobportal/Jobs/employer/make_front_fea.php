...
$sb_total_cost=$sbrow_con["sb_front_featured_fee"];
...
{
	header("Location: gen_confirm_mem.php?errmsg=".urlencode("You must have atleast $sb_total_cost $sb_fee_currency_name in account to make your job appear as front page featured, please add some money first."));
	die();
}
	...
	$sbq_job="update `sbjbs_jobs` set sb_front_featured='yes' where sb_id=$sb_id and sb_uid=$sb_uid";
	mysql_query($sbq_job);
	if(mysql_affected_rows() == 1)
	{
///----------adding transaction
		if( ($sbrow_con["sb_front_featured_fee"] > 0) )
			{
		...
		header("Location: manage_jobs.php?msg=".urlencode("Job has been made front page featured."));
		die();
	}
	else
	{
		header("Location: gen_confirm_mem.php?errmsg=".urlencode("Unable to make job front page featured, please try again."));
		die();
	}
