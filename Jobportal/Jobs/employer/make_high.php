...
$sb_total_cost=$sbrow_con["sb_highlight_fee"];
...
{
	header("Location: gen_confirm_mem.php?errmsg=".urlencode("You must have atleast $sb_total_cost $sb_fee_currency_name in account to make your job appear as highlighted, please add some money first."));
	die();
}
	...
	$sbq_job="update `sbjbs_jobs` set sb_highlight='yes' where sb_id=$sb_id and sb_uid=$sb_uid";
	mysql_query($sbq_job);
...
///----------adding transaction
		if( ($sb_total_cost > 0) )
			{
				$sbqi_tra="insert into sbjbs_transactions (sb_uid, sb_amount, sb_description, sb_date_submitted) values ($sb_uid, ".-$sb_total_cost.", 'Made your job ''$sb_title'' to appear as Highlighted', $sb_date)";
				//die($sbqi_tra);
				mysql_query($sbqi_tra);
			}

		header("Location: manage_jobs.php?msg=".urlencode("Job has been made highlighted."));
		die();
	}
	else
	{
		header("Location: gen_confirm_mem.php?errmsg=".urlencode("Unable to make job highlighted, please try again."));
		die();
	}
