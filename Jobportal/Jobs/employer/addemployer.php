...
if(count($_POST)<>0)		//IF SOME FORM WAS POSTED DO VALIDATION
{
	...
	elseif(mysql_num_rows(mysql_query("select * from sbjbs_employers where sb_username='$username' or sb_email_addr='$email'"))!= 0)
	{
			$errs[$errcnt]="Some employer with same Username or Email Address already exists";
    		$errcnt++;
	}
...
//	{ $mem_type=$config["sb_memtype"];}
	...
	( sb_username ,sb_password ,sb_last_login, sb_signup_on ,sb_suspended, sb_title, sb_firstname , sb_lastname, sb_email_addr , sb_addr1 , sb_city , sb_state , sb_zip , sb_country , sb_telephone, sb_mobile)
VALUES 	( '$username' ,'$password',0,'".date("YmdHis",time())."','$suspended', '$sb_title', '$firstname' , '$lastname', '$email','$street','$city','$state','$zip_code' ,".$_REQUEST["country"].",'$phone_no', '$mobile')
";
//echo $query_insert;
//die();
...
//	die();
		...
		//mail ends here
		header("Location: signin_emp.php?errmsg=".urlencode("You are successfully registered with us"));
		die();
		}
		else
		{
...
else
{
	...
	$rs0_query=mysql_query ("select * from sbjbs_emp_signups where sb_email='$email' and sb_rnum='$rnum' ");
...
{
?>
<table width="80%" border="0" align="center" cellpadding="2" cellspacing="0" class="errorstyle">
  <tr>
    <td><strong>&nbsp;Invalid 
      Access</strong></td>
  </tr>
...
<form name="form1" method="post" action="addemployer.php" onSubmit="return validate(this);">
